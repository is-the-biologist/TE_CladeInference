...
class subfamilyInference:
    ...
    def PCA_plot(self, TE, outdir='/Users/iskander/Documents/Barbash_lab/TE_diversity_data/PCA_PLOTS', save_plot=False):
        ...
        with sns.axes_style("whitegrid"):
            ...
            axs = fig.subplots(1)
            sns.scatterplot(data=PCA_matrix, x="PC1", y="PC2", hue="Population", palette=self.color_map, edgecolor=None, alpha=1, s=100, ax=axs)
            axs.set_aspect('equal', adjustable='box')

            plt.xticks(fontsize=20)
            plt.xlabel(f"PC1 ({EV[0]*100:0.2f}%)", fontsize=25)
            plt.ylabel(f"PC2 ({EV[1]*100:0.2f}%)", fontsize=25)
...
class haplotypeAnalysis:
    ...
    def populationHaplotypes(self, TE, cluster_ID, saveplot=False):
        ...
        with sns.axes_style('whitegrid'):
            ...
            axs = fig.subplots(1)
            #axs.set_aspect('equal', adjustable='box')
            sns.boxplot(data=clusters, x='POP', y=cluster_ID, saturation=1, palette=self.color_map, ax=axs)
            ...
            plt.xticks(fontsize=20, labels=["B", "I", "N", "T", "Z"], ticks=[0,1,2,3,4])
            plt.yticks(fontsize=20)
            fig.tight_layout()
            if saveplot:
    ...
    def plot_KW(self, read_table=True, filter_haplos=0, saveplot=False):
        ...
        with sns.axes_style('whitegrid'):
            ...
            plt.xlabel('')
            plt.legend(fontsize=12.5, markerscale=2, edgecolor='black', loc="best")
...
class piRNA:
    ...
    def piRNA_correlation(self, TE, cluster, saveplot=False, scale="linear", abline=True, pop=True):
        ...
        with sns.axes_style("whitegrid"):
            ...
            plt.ylabel("piRNA read depth", size=25)
            plt.xlabel("Copy Number", size=25)
